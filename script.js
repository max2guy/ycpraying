...
let members = [];
function loadData() {
    Promise.all([membersRef.once('value'), centerNodeRef.once('value')]).then(([mSnap, cSnap]) => {
        const mData = mSnap.val(); if (mData) members = Object.keys(mData).map(key => ({ firebaseKey: key, ...mData[key] }));
        isFirstRender = false; document.getElementById('loading').classList.add('hide'); updateGraph();
    });
}
loadData();
...
simulation = d3.forceSimulation().force("link", d3.forceLink().id(d => d.id).distance(140)).force("charge", d3.forceManyBody().strength(-400)).force("center", d3.forceCenter(width/2, height/2));

function updateGraph() {
    const nodes = [{id:"center", name:"연천교회", type:"root"}, ...members];
    ...
    nEnter.append("circle").attr("r", d => d.type==="root" ? 50 : 30).attr("fill", d => d.color || "#ccc");
    nEnter.append("text").text(d => d.name).attr("text-anchor", "middle").attr("dy", ".35em");
    node = nEnter.merge(node);
    simulation.nodes(nodes).on("tick", () => {
        node.attr("transform", d => `translate(${d.x},${d.y})`);
        linkGroup.selectAll("line").data(links).join("line").attr("x1", d => d.source.x).attr("y1", d => d.source.y).attr("x2", d => d.target.x).attr("y2", d => d.target.y).attr("stroke", "#fff");
    });
...
function addNewMember() {
    const n = prompt("이름:");
    if(n) membersRef.push({id:"m_"+Date.now(), name:n, prayers:[]}).then(() => location.reload());
}

async function fetchWeather() {
    document.getElementById('weather-text').innerText = "연천군: 맑음, 5°C";
}
fetchWeather();
